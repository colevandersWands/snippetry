...
import { evaluate } from '../public/lib/scheme-dot-json/index.js';
import { simplit } from './simplit.mjs';

import { REPO, SEPARATOR, HREF } from '../public/src/CONSTANTS.js';
import { state } from '../public/src/state.js';
...
import {
  ...
  renderLinks,
  renderMarkdown,
} from '../public/src/utils/index.js';
...
const renderCode = (snippet) => {
  if (snippet.name.endsWith('.md')) {
    ...
    container.innerHTML = `<div class='wrappable'>${renderMarkdown(snippet.code)}</div>`;
...
<div class="snippet-header">
  ${
    snippet.alt
      ? `<details>
          <summary><h2 id="${snippet.name.toLowerCase()}" style="display: inline;">${
          snippet.name
        }</h2></summary>
          
          <p><em>${snippet.alt}</em></p>
  ...
  <div class="down">
    ...
    const linkTypes = ['forelinks', 'metalinks', 'aftlinks'];
    const hasLinks = linkTypes.some((linkType) => linkType in snippet);
    if (hasLinks) {
      let interLinks = '';
      for (const linkType of linkTypes) {
        if (!(linkType in snippet)) continue;
        interLinks += renderLinks(snippet[linkType], linkType);
      }
      const linksContainer = document.createElement('div');
      linksContainer.innerHTML = `<hr><hr><div class="interlinks-container">${interLinks}</div>`;
      snippet.containerHighlight.appendChild(linksContainer);
    }
...
const replaceWithEditor = (snippet) => {
  ...
  if (snippet.name.endsWith('js')) {
    ...
    snippet.}.className = 'editor match-braces language-text';
