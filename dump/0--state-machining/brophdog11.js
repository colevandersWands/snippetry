// https://github.com/brophdawg11/state-machine/blob/main/src/state-machine.js
...
function has(obj, key) {
  if (obj == null || typeof obj !== 'object' || key == null || typeof key !== 'string') {
    return false;
  }
...
module.exports = class StateMachine {
  ...
  _setState(state, transition, data) {
    ...
    return new Promise((resolve, reject) => {
      if (
        this._machine.states[state] &&
        isFunction(this._machine.states[state].onEnter)
      ) {
        const retVal = this._machine.states[state].onEnter(
          data,
          state,
          oldState,
          transition,
  ...
  getDotFile() {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      const transitions = stateNames
        .reduce((acc, from) => {
          const stateTransitions =
            this._machine.states[from].transitions || /* istanbul ignore next */ {};
          const str = Object.entries(stateTransitions)
            .reduce(
