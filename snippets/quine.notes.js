...

{
  ...
  })('split', ',join,call,map', ",()'\\", '1821383038303012434430383082', 0);
  ...
  q = "'";
  s = 'q="";s=;f=s.substr.bind(s);console.log(f(0,3)+q+f(3,4)+q+s+q+f(7));';
  f = s.substr.bind(s);
  console.log(f(0, 3) + q + f(3, 4) + q + s + q + f(7));

  (() => {
    var arr = [
      '(()=>{var arr=',
      0,
      ';arr[1]=JSON.stringify(arr);return arr.join("");})()',
    ];
    arr[1] = JSON.stringify(arr);
    return arr.join('');
  })();

  a =
    'r=(s,p,q)=>s.replaceAll(p,q);b="\'"+r(r(a,"\\\\","\\\\\\\\"),"\\\'","\\\\\'")+"\'";console.log("a="+b+";"+a);';
  r = (s, p, q) => s.replaceAll(p, q);
  b = "'" + r(r(a, '\\', '\\\\'), "'", "\\'") + "'";
  console.log('a=' + b + ';' + a);
  ...
  // Never ending eval...
  let x =
    'let x =%22quinner%22;let y=decodeURI(x).replace(/quinner/, x);alert(y);eval(y);';
  let y = decodeURI(x).replace(/quinner/, x);
  alert(y);
  eval(y);
}
